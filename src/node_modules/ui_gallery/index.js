...
async function ui_gallery (opts = {}) {
  ...
  async function create_component (entries_obj) {
    ...
    for (const [name_raw, factory] of entries_obj) {
      // CRITICAL: Force the name to a string. 
      // If 'require' keys are Symbols, template literals or concatenation will crash.
      const name = String(name_raw) 
      ...
      // Safety check: ensure subscription data exists for this index
      const sub_state = subs[index] || { sid: opts.sid + '/missing/' + name }

      try {
        // Route initialization based on component name string
        if (name === 'theme_widget') {
          component_content = await factory({ ...sub_state }, theme_widget_io.invite('theme_widget', { up: id }))
        } else {
          // Use the sanitized string 'name' for the network protocol
          const component_io = preview_ios[name] || theme_widget_io
          component_content = await factory({ ...sub_state }, component_io.invite(name, { up: id }))
        }
        ...
        // Admin and Editor logic - Safe ID resolution
        const sid_path = sub_state.sid
        const node_id = admin.status.s2i[sid_path]

        if (node_id) {
          const editor_index = index + 1
          const component_editor = editor_subs[editor_index] ? await editor(editor_subs[editor_index]) : null
          
          if (component_content) inner.append(component_content)
          if (component_editor) inner.append(component_editor)

          // State data collection for the Editor
          const result = {}
          const drive_data = admin.status.dataset.drive
          const modulepath = String(node_id).split(':')[0]
          ...
          for (const node of nodes) {
            result[node] = {}
            const datasets = drive_data.list('', node)
            for (const dataset of datasets) {
              result[node][dataset] = {}
              const files = drive_data.list(dataset, node)
              for (const file of files) {
                const file_data = await drive_data.get(dataset + file, node)
          ...
          if (editor_subs[editor_index]) {
            const editor_sid = editor_subs[editor_index].sid
            const editor_internal_id = admin.status.a2i[admin.status.s2i[editor_sid]]
            const port = await item.get(editor_internal_id)
            if (port) port.postMessage(result)
          }
        } else {
          // Fallback if no admin mapping exists
          if (component_content) inner.append(component_content)
        }
      } catch (err) {
        console.error(`[ui_gallery] Error loading component "${name}":`, err)
      }
